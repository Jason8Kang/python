...
class KNearestNeighbor:
  ...
  def compute_distances_no_loops(self, X):
    ...
    print a,'\n',b
    a_sum_square=(np.sum(a**2,axis=1)).reshape(a.shape[0],1)
    b_sum_square=(np.sum(b**2,axis=1))
    print np.tile(a_sum_square,[b.shape[0],1]).T
    print np.tile(b_sum_square,[a.shape[0],1])
    #print a_sum_square,b_sum_square
    a_sum_square+b_sum_square-2*np.dot(a,b.T)
    '''
    X_sum_square=(np.sum(X**2,axis=1)).reshape(X.shape[0],1)
    X_train_sum_square=(np.sum(self.X_train**2,axis=1))
    
    dists=X_sum_square+X_train_sum_square-2*np.dot(X,self.X_train.T)
    #########################################################################
    #                         END OF YOUR CODE                              #
    #########################################################################
    return dists
  ...
  def predict_labels_copy(self, dists, k=1):
    """
    Given a matrix of distances between test points and training points,
    predict a label for each test point.
    ...
    for i in xrange(num_test):
      ...
      # label.                                                                #
      #########################################################################
      y_pred[i]=np.argmax(np.bincount(self.y_train[index]))
      #########################################################################
      #                           END OF YOUR CODE                            # 
      #########################################################################
